...
Shooter::Shooter() {
    ...
    m_shooterHeightPID = std::make_shared<LeverPIDController>(
        k_heightShooterP, k_heightShooterI, k_heightShooterD, k_heightShooterV,
        k_heightShooterA, k_heightShooterF, &m_shooterHeightGrbx,
        &m_shooterHeightGrbx, 0.02);
    ...
    m_shooterHeightProfile = std::make_shared<TrapezoidProfile>(
        m_shooterHeightPID, k_shooterHeightMaxSpeed, k_shooterTimeMaxV);
    ...
    // Sets encoder type
    m_leftShootGrbx->SetFeedbackDevice(CANTalon::CtreMagEncoder_Relative);
    m_rightShootGrbx->SetFeedbackDevice(CANTalon::CtreMagEncoder_Relative);
    m_shooterHeightGrbx.SetFeedbackDevice(CANTalon::CtreMagEncoder_Relative);

    m_shooterHeightPID->Enable();

    m_joystickTimer.Start();

    m_joystickEvent.RegisterButtonEvent("PressedIntakeButton", k_shootStickPort,
                                        2, true);
    ...
    m_dioEvent.RegisterInputEvent("BallLoaded", k_shooterIntakeLimitChannel,
                                  true, false, m_shootSM);
    m_dioEvent.RegisterInputEvent("ShooterZeroed", k_shooterBottomLimitChannel,
                                  true, false, m_shootSM);
    ...
    state->Entry = [this] { m_timerEvent.Reset(); };
    state->Run = [this] {
...
void Shooter::SetManualOverride(bool manual) { m_manual = manual; }
...
bool Shooter::GetManualOverride() const { return m_manual; }
...
void Shooter::SetShooterHeight(double height, bool increment) {
    if ((m_shooterHeight.displacement >= k_shooterHeightMax && height > 0) ||
        ...
        if (increment == true) {
            ...
            m_shooterHeight.displacement +=
                m_shooterHeight.velocity * m_joystickTimer.Get();
            if (m_shooterHeightPID->GetSetpoint() != m_shooterHeight) {
                m_shooterHeightPID->SetSetpoint(m_shooterHeight);
            }
        } else {
