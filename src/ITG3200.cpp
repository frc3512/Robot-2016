...
void ITG3200::Calibrate() {
    ...
    for (int count = 0; count < k_numCalibrationSamples; count++) {
        ...
        std::cout << "< " << GetRawX() << " , " << GetRawY() << " , " <<
        GetRawZ() << " >" << std::endl;
    }

    m_xZero = static_cast<double>(sumX) / k_numCalibrationSamples;
    m_yZero = static_cast<double>(sumY) / k_numCalibrationSamples;
    m_zZero = static_cast<double>(sumZ) / k_numCalibrationSamples;

    std::cout << "Zeroed Z " << m_zZero << std::endl;
}
