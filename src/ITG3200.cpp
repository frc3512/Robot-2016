...
ITG3200::ITG3200(frc::I2C::Port port, uint32_t deviceAddress)
    : m_i2c(port, deviceAddress) {
    // Sets gyro at +-2000deg/sec and 98Hz low pass filter
    m_i2c.Write(k_digitalLowPassFilterRegister,
                DLPF_CFG_1 | DLPF_FS_SEL_0 | DLPF_FS_SEL_1);

    // Sets sample rate to 100Hz
    m_i2c.Write(k_samplePeriodDividerRegister, 0x09);
...
double ITG3200::GetAngleX() const { return m_xAngle; }

double ITG3200::GetAngleY() const { return m_yAngle; }

double ITG3200::GetAngleZ() const { return m_zAngle; }
...
void ITG3200::Calibrate() {
    ...
    for (int count = 0; count < k_numCalibrationSamples; count++) {
        ...
        std::this_thread::sleep_for(
            std::chrono::duration<double>(k_samplePeriod));
    }
...
int16_t ITG3200::GetRawX() {
    ...
    if (m_i2c.Read(k_gyroXOutRegister, 2, data)) {
...
int16_t ITG3200::GetRawY() {
    ...
    if (m_i2c.Read(k_gyroYOutRegister, 2, data)) {
        std::cout << "I2C: read failed" << std::endl;
        return m_yZero;
    } else {
        return (data[0] << 8) | data[1];
...
int16_t ITG3200::GetRawZ() {
    ...
    if (m_i2c.Read(k_gyroZOutRegister, 2, data)) {
        std::cout << "I2C: read failed" << std::endl;
        return m_zZero;
    } else {
        return (data[0] << 8) | data[1];
