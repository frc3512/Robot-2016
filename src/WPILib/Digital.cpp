...
SPIAccumulator* spiAccumulators[5] =
...
void initializeDigital(int32_t* status) {
    if (digitalSystemsInitialized) {
        return;
    }

    hal::Resource::CreateResourceObject(&DIOChannels,
                                        tDIO::kNumSystems * kDigitalPins);
    hal::Resource::CreateResourceObject(&DO_PWMGenerators,
    ...
    pwmSystem->writeConfig_Period(
        (uint16_t) (kDefaultPwmPeriod / loopTime + .5),
        status);
    uint16_t minHigh =
        (uint16_t) ((kDefaultPwmCenter - kDefaultPwmStepsDown * loopTime) /
                    loopTime + .5);
...
void setPWM(void* digital_port_pointer, unsigned short value, int32_t* status) {
    ...
    if (port->port.pin < tPWM::kNumHdrRegisters) {
        ...
        pwmSystem->writeMXP(port->port.pin - tPWM::kNumHdrRegisters,
                            value,
                            status);
    }
}
...
unsigned short getPWM(void* digital_port_pointer, int32_t* status) {
    ...
    if (port->port.pin < tPWM::kNumHdrRegisters) {
        ...
        return pwmSystem->readMXP(port->port.pin - tPWM::kNumHdrRegisters,
                                  status);
    }
}
 ...
 */
void setPWMPeriodScale(void* digital_port_pointer,
    ...
    if (port->port.pin < tPWM::kNumPeriodScaleHdrElements) {
        ...
        pwmSystem->writePeriodScaleMXP(
            port->port.pin - tPWM::kNumPeriodScaleHdrElements,
            squelchMask,
            status);
    }
}
...
void setPWMRate(double rate, int32_t* status) {
    ...
    uint8_t pwmPeriodPower =
        (uint8_t) (log(1.0 /
                       (pwmSystem->readLoopTiming(status) * 0.25E-6 * rate)) /
                   log(2.0) + 0.5);
...
void setPWMDutyCycle(void* pwmGenerator, double dutyCycle, int32_t* status) {
    ...
    }
    {
        ...
        if (id < 4) {
            digitalSystem->writePWMDutyCycleA(id, (uint8_t) rawDutyCycle,
                                              status);
        }
        else {
            digitalSystem->writePWMDutyCycleB(id - 4,
                                              (uint8_t) rawDutyCycle,
                                              status);
        }
    }
}
...
bool allocateDIO(void* digital_port_pointer, bool input, int32_t* status) {
...

    {
        ...
        tDIO::tOutputEnable outputEnable = digitalSystem->readOutputEnable(
            status);
...
bool allocatePWMChannel(void* digital_port_pointer, int32_t* status) {
    ...
    if (port->port.pin > tPWM::kNumHdrRegisters - 1) {
        ...
        if (DIOChannels->Allocate(remapMXPPWMChannel(port->port.pin) + 10,
                                  buf) == ~0ul) {
            return false;
        }
        uint32_t bitToSet = 1 << remapMXPPWMChannel(port->port.pin);
        short specialFunctions = digitalSystem->readEnableMXPSpecialFunction(
            status);
        digitalSystem->writeEnableMXPSpecialFunction(
            specialFunctions | bitToSet,
            status);
...
void setDIO(void* digital_port_pointer, short value, int32_t* status) {
    ...
    }
    {
        ...
        if (port->port.pin < kNumHeaders) {
            if (value == 0) {
                currentDIO.Headers = currentDIO.Headers &
                                     ~(1 << port->port.pin);
            }
            else if (value == 1) {
            ...
            if (value == 0) {
                currentDIO.MXP = currentDIO.MXP &
                                 ~(1 << remapMXPChannel(port->port.pin));
            }
            else if (value == 1) {
                currentDIO.MXP = currentDIO.MXP |
                                 (1 << remapMXPChannel(port->port.pin));
...
bool getDIODirection(void* digital_port_pointer, int32_t* status) {
    DigitalPort* port = (DigitalPort*) digital_port_pointer;
    tDIO::tOutputEnable currentOutputEnable = digitalSystem->readOutputEnable(
        status);
    ...
    if (port->port.pin < kNumHeaders) {
        ...
        return ((currentOutputEnable.MXP >> remapMXPChannel(port->port.pin)) &
                1) != 0;
    }
}
...
void pulse(void* digital_port_pointer, double pulseLength, int32_t* status) {
    ...
    digitalSystem->writePulseLength((uint8_t) (1.0e9 * pulseLength /
                                               (pwmSystem->readLoopTiming(status)
                                                * 25)), status);
...
bool isPulsing(void* digital_port_pointer, int32_t* status) {
    ...
    if (port->port.pin < kNumHeaders) {
        ...
        return (pulseRegister.MXP & (1 << remapMXPChannel(port->port.pin))) !=
               0;
    }
}
 ...
 */
void setFilterSelect(void* digital_port_pointer,
                     int filter_index,
                     int32_t* status) {
    DigitalPort* port = (DigitalPort*) digital_port_pointer;
    ...
    std::lock_guard<priority_recursive_mutex> sync(digitalDIOMutex);
    if (port->port.pin < kNumHeaders) {
        digitalSystem->writeFilterSelectHdr(port->port.pin, filter_index,
                                            status);
    }
    else {
...
int getFilterSelect(void* digital_port_pointer, int32_t* status) {
    ...
    if (port->port.pin < kNumHeaders) {
        ...
        return digitalSystem->readFilterSelectMXP(remapMXPChannel(
                                                      port->port.pin),
                                                  status);
    }
}
...
uint32_t getFilterPeriod(int filter_index, int32_t* status) {
    ...
    uint32_t mxp_period = 0;
    {
        ...
        if (*status == 0) {
            mxp_period =
                digitalSystem->readFilterPeriodMXP(filter_index, status);
        }
    }
 ...
 */
extern "C++" void remapDigitalSource(bool analogTrigger,
                                     uint32_t& pin,
                                     uint8_t& module) {
    if (analogTrigger) {
        module = pin >> 4;
 ...
 */
void setCounterUpSource(void* counter_pointer,
                        uint32_t pin,
                        bool analogTrigger,
                        int32_t* status) {
    Counter* counter = (Counter*) counter_pointer;
 ...
 */
void setCounterUpSourceEdge(void* counter_pointer,
                            bool risingEdge,
                            bool fallingEdge,
                            int32_t* status) {
    Counter* counter = (Counter*) counter_pointer;
 ...
 */
void setCounterDownSource(void* counter_pointer,
                          uint32_t pin,
                          bool analogTrigger,
                          int32_t* status) {
    Counter* counter = (Counter*) counter_pointer;
    ...
    counter->counter->writeConfig_DownSource_AnalogTrigger(analogTrigger,
                                                           status);
 ...
 */
void setCounterDownSourceEdge(void* counter_pointer,
                              bool risingEdge,
                              bool fallingEdge,
                              int32_t* status) {
    Counter* counter = (Counter*) counter_pointer;
 ...
 */
void setCounterSemiPeriodMode(void* counter_pointer,
                              bool highSemiPeriod,
                              int32_t* status) {
    Counter* counter = (Counter*) counter_pointer;
 ...
 */
void setCounterPulseLengthMode(void* counter_pointer,
                               double threshold,
                               int32_t* status) {
    Counter* counter = (Counter*) counter_pointer;
    counter->counter->writeConfig_Mode(kPulseLength, status);
    counter->counter->writeConfig_PulseLengthThreshold((uint32_t) (threshold *
                                                                   1.0e6) * kSystemClockTicksPerMicrosecond,
                                                       status);
}
 ...
 */
void setCounterSamplesToAverage(void* counter_pointer,
 ...
 * seconds.
 */
void setCounterMaxPeriod(void* counter_pointer,
                         double maxPeriod,
                         int32_t* status) {
    Counter* counter = (Counter*) counter_pointer;
    counter->counter->writeTimerConfig_StallPeriod(
        (uint32_t) (maxPeriod * 4.0e8),
        status);
}
 ...
 */
void setCounterUpdateWhenEmpty(void* counter_pointer,
                               bool enabled,
                               int32_t* status) {
    Counter* counter = (Counter*) counter_pointer;
 ...
 */
void setCounterReverseDirection(void* counter_pointer,
...
void* initializeEncoder(uint8_t port_a_module,
                        uint32_t port_a_pin,
                        bool port_a_analog_trigger,
                        uint8_t port_b_module,
                        uint32_t port_b_pin,
 ...
 */
void setEncoderMaxPeriod(void* encoder_pointer,
                         double maxPeriod,
                         int32_t* status) {
    Encoder* encoder = (Encoder*) encoder_pointer;
    encoder->encoder->writeTimerConfig_StallPeriod((uint32_t) (maxPeriod *
                                                               4.0e8 *
                                                               DECODING_SCALING_FACTOR),
                                                   status);
}
 ...
 */
void setEncoderReverseDirection(void* encoder_pointer,
                                bool reverseDirection,
                                int32_t* status) {
    Encoder* encoder = (Encoder*) encoder_pointer;
    encoder->encoder->writeConfig_Reverse(reverseDirection, status);
 ...
 * encoder's count.
 */
void setEncoderIndexSource(void* encoder_pointer,
                           uint32_t pin,
                           bool analogTrigger,
                           bool activeHigh,
                           bool edgeSensitive,
                           int32_t* status) {
    Encoder* encoder = (Encoder*) encoder_pointer;
    encoder->encoder->writeConfig_IndexSource_Channel((unsigned char) pin,
                                                      status);
...
void spiInitialize(uint8_t port, int32_t* status) {
    ...
    case 4:
        ...
        digitalSystem->writeEnableMXPSpecialFunction(digitalSystem->readEnableMXPSpecialFunction(
                                                         status) | 0x00F0,
                                                     status);
 ...
 */
int32_t spiTransaction(uint8_t port,
                       uint8_t* dataToSend,
                       uint8_t* dataReceived,
                       uint8_t size) {
    std::lock_guard<priority_recursive_mutex> sync(spiGetSemaphore(port));
    return spilib_writeread(spiGetHandle(
                                port), (const char*) dataToSend, (char*) dataReceived,
                            (int32_t) size);
}
...
int32_t spiWrite(uint8_t port, uint8_t* dataToSend, uint8_t sendSize) {
    std::lock_guard<priority_recursive_mutex> sync(spiGetSemaphore(port));
    return spilib_write(spiGetHandle(
                            port), (const char*) dataToSend,
                        (int32_t) sendSize);
}
 ...
 */
void spiSetOpts(uint8_t port,
                int msb_first,
                int sample_on_trailing,
                int clk_idle_high) {
    std::lock_guard<priority_recursive_mutex> sync(spiGetSemaphore(port));
    spilib_setopts(spiGetHandle(
                       port), msb_first, sample_on_trailing, clk_idle_high);
}
...
void spiSetChipSelectActiveHigh(uint8_t port, int32_t* status) {
    std::lock_guard<priority_recursive_mutex> sync(spiGetSemaphore(port));
    if (port < 4) {
        spiSystem->writeChipSelectActiveHigh_Hdr(spiSystem->readChipSelectActiveHigh_Hdr(
                                                     status) | (1 << port),
                                                 status);
    }
    else {
...
static void spiAccumulatorProcess(uint64_t currentTime, void* param) {
    ...
    spilib_writeread(spiGetHandle(
                         accum->port), (const char*) accum->cmd, (char*) resp_b,
                     (int32_t) accum->xfer_size);
 ...
 */
void spiInitAccumulator(uint8_t port,
                        uint32_t period,
                        uint32_t cmd,
                        uint8_t xfer_size,
                        uint32_t valid_mask,
    ...
    if (!accum->notifier) {
        accum->notifier = initializeNotifier(spiAccumulatorProcess,
                                             accum,
                                             status);
 ...
 */
void spiGetAccumulatorOutput(uint8_t port,
                             int64_t* value,
                             uint32_t* count,
                             int32_t* status) {
    std::lock_guard<priority_recursive_mutex> sync(spiGetSemaphore(port));
...
void i2CInitialize(uint8_t port, int32_t* status) {
    ...
    priority_recursive_mutex& lock = port ==
                                     0 ? digitalI2COnBoardMutex :
                                     digitalI2CMXPMutex;
    {
        std::lock_guard<priority_recursive_mutex> sync(lock);
        if (port == 0) {
            i2COnboardObjCount++;
            if (i2COnBoardHandle > 0) {
                return;
            }
            ...
            if (!allocateDIO(getPort(24), false, status)) {
                return;
            }
            if (!allocateDIO(getPort(25), false, status)) {
                return;
            }
            digitalSystem->writeEnableMXPSpecialFunction(digitalSystem->readEnableMXPSpecialFunction(
                                                             status) | 0xC000,
 ...
 */
int32_t i2CTransaction(uint8_t port,
    ...
    {
        std::lock_guard<priority_recursive_mutex> sync(lock);
        return i2clib_writeread(handle,
                                deviceAddress,
