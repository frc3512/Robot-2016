...
public:
    ...
    enum FeedbackDeviceStatus {
        FeedbackStatusUnknown = 0,  // !< Sensor status could not be determined.
     ...
     *   Talon will stop processing motion profile points.  This means the
     * buffer is
     ...
     * IsUnderrun.
     * Hold will signal Talon keep processing the active trajectory
     * indefinitely.
     *   If the active traj is cleared, Talon will neutral motor drive.
     * Otherwise
     ...
     *   If MP finished gracefully set(Hold) to hold position servo and
     * disconnect buffer,
    ...
    struct TrajectoryPoint {
        ...
        double velocity;  // !< The velocity to feed-forward.
                           ...
                           * then Talon will default to 1ms.  If value exceeds 255ms API will cap
                           * it.
                           */
         ...
         * Typically this is hardcoded to the a particular slot, but you are
         * free
         ...
         * the
         * PID gains, however this is synchronous the streaming, and doesn't
         * require restoing
         * gains when finished.
         *
    ...
    struct MotionProfileStatus {
         ...
         * applicaion
         * can dump several points at once.  The API will then stream them into
         * the Talon's
         ...
         * enabled, or hold).
         * When changing the set() value in MP mode, it's important to check
         * this signal to
         * confirm the change takes effect before interacting with the top
         * buffer.
         */
    ...
    virtual void SetPID(double p, double i, double d, double v,
                        double a) override;
    ...
    void ConfigNominalOutputVoltage(double forwardVoltage,
     ...
     * disable.
     * @param risingEdge  boolean input, pass true to clear the position on
     * rising edge,
    ...
    PIDState GetSetpoint() const override;

    /**
     * Calling application can opt to speed up the handshaking between the robot
     * API and the Talon to increase the
     * download rate of the Talon's Motion Profile.  Ideally the period should
     * be no more than half the period
     ...
     * @return true  if trajectory point push ok. CTR_BufferFull if buffer is
     * full
...
private:
    ...
    int m_profile =
        0;  // Profile from CANTalon to use. Set to zero until we can
    // actually test this.
     ...
     * Caller can use ConfigEncoderCodesPerRev to set the quadrature encoder
     * CPR.
     */
    ...
    /**
     * @param devToLookup FeedbackDevice to lookup the scalar for.  Because
     * Talon
     ...
     *                            If user has never called the config routine
     * for the selected sensor, then the caller
     *                            is likely passing rotations in engineering
     * units already, in which case it is returned
     *                            as is.
     *                            @see ConfigPotentiometerTurns
     *                            @see ConfigEncoderCodesPerRev
    ...
    /**
     * @param rpm     double precision value representing number of rotations
     * per minute of selected feedback sensor.
     *                            If user has never called the config routine
     * for the selected sensor, then the caller
     *                            is likely passing rotations in engineering
     * units already, in which case it is returned
     ...
     * Talon SRX units.
     *                            If user has never called the config routine
     * for the selected sensor, then the return
     *                            will be in TALON SRX units as well to match
     * the behavior in the 2015 season.
     *                            @see ConfigPotentiometerTurns
     *                            @see ConfigEncoderCodesPerRev
